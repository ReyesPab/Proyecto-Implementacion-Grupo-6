...
async function verificarCodigo2FA(e) {
    ...
    try {
        ...
        if (result.status === '200') {
            ...
            if (result.data && result.data.data) {
                ...
                // GUARDAR EL NOMBRE DEL USUARIO - MODIFICACIÓN IMPORTANTE
    ...
    document.getElementById('loginForm').addEventListener('submit', async (e) => {
        ...
        if (!/^[A-Z0-9ÁÉÍÓÚÑÜ]+$/.test(usuario)) {
            showAlert('El usuario solo puede contener letras, números y caracteres válidos (sin símbolos especiales)', 'error');
            return;
        }
        
        // Validar contraseña en frontend (SOLO VALIDACIONES BÁSICAS)
        ...
        try {
            ...
            if (result.data && result.data.data) {
                ...
                const nombreUsuario = userData.user?.NOMBRE_USUARIO || 
                                    userData.nombre_usuario || 
                                    userData.usuario || 
                                    usuario; // Usar el usuario ingresado como fallback
                sessionStorage.setItem('user_name', nombreUsuario);
                console.log("Nombre de usuario guardado:", nombreUsuario);
            }
