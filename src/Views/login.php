    ...
    try {
        ...
        if (result.status === '200') {
            ...
            if (userData.requiere_2fa) {
                ...
                console.log("Login directo - No requiere 2FA");
                ...
                const nombreUsuario = userData.user?.NOMBRE_USUARIO || usuario;
                sessionStorage.setItem('user_name', nombreUsuario);
                console.log("Nombre de usuario guardado:", nombreUsuario);
                console.log("Primer ingreso:", userData.primer_ingreso);
                
// REDIRECCIÓN CORREGIDA - VERIFICACIÓN MEJORADA
setTimeout(() => {
    console.log("🔍 Analizando datos para redirección:", userData);
    
    // Obtener datos de la sesión
    const sessionData = result.data.session_data || userData;
    
    // Verificar primer_ingreso de múltiples formas posibles
    const primerIngreso = sessionData.primer_ingreso || 
                         userData.primer_ingreso || 
                         userData.PRIMER_INGRESO || 
                         userData.primerIngreso;
    
    console.log("🔍 Valor de primer_ingreso:", primerIngreso, "Tipo:", typeof primerIngreso);
    
    // Convertir a booleano para comparación consistente
    // PRIMER_INGRESO = 0 significa que es primer ingreso y debe cambiar contraseña
    // PRIMER_INGRESO = 1 significa que ya cambió contraseña y puede ir al dashboard
    const esPrimerIngreso = primerIngreso === 0 || 
                           primerIngreso === '0' || 
                           primerIngreso === false;
    
    console.log("🔍 ¿Es primer ingreso?:", esPrimerIngreso);
    console.log("🔍 Estado del usuario:", sessionData.estado_usuario);
    
    // Guardar en sessionStorage para uso posterior
    sessionStorage.setItem('user_id', sessionData.user_id);
    sessionStorage.setItem('user_name', sessionData.user_name);
    sessionStorage.setItem('primer_ingreso', esPrimerIngreso ? '0' : '1');
    sessionStorage.setItem('estado_usuario', sessionData.estado_usuario);
    
    if (esPrimerIngreso) {
        console.log("🎯 REDIRIGIENDO a cambiar-password - Primer ingreso detectado");
        window.location.href = '/sistema/public/cambiar-password';
    } else {
        console.log("🎯 REDIRIGIENDO a inicio - Usuario activo");
        window.location.href = '/sistema/public/inicio';
    }
}, 1000);
            }
