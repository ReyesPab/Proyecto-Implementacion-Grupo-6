...
class DashboardModel {
    ...
    public function obtenerEstadisticasGenerales() {
        try {
            ...
            // Usuarios activos (con sesión reciente en las últimas 24 horas)
            ...
            // Usuarios bloqueados
            $query = "SELECT COUNT(*) as total FROM TBL_MS_USUARIOS WHERE ESTADO_USUARIO = 'Bloqueado'";
            $stmt = $this->conn->prepare($query);
            $stmt->execute();
            $stats['usuarios_bloqueados'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];

            // Usuarios nuevos (que no han iniciado sesión por primera vez)
            $query = "SELECT COUNT(*) as total FROM TBL_MS_USUARIOS WHERE ESTADO_USUARIO = 'Nuevo'";
            $stmt = $this->conn->prepare($query);
            $stmt->execute();
            $stats['usuarios_nuevos'] = $stmt->fetch(PDO::FETCH_ASSOC)['total'];

            // Actividades de hoy (bitácora del día actual)
...
public function obtenerReporteComprasSencillo() {
    try {
        ...
        $stmt = $this->conn->prepare("CALL SP_OBTENER_REPORTE_COMPRAS_SENCILLO()");
        $stmt->execute();
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        return [
            'compras_hoy' => (float)$result['compras_hoy'],
            'cantidad_hoy' => (int)$result['cantidad_hoy'],
            'compras_semana' => (float)$result['compras_semana'],
            'cantidad_semana' => (int)$result['cantidad_semana'],
            'compras_quincena' => (float)$result['compras_quincena'],
            'cantidad_quincena' => (int)$result['cantidad_quincena'],
            'compras_mes' => (float)$result['compras_mes'],
            'cantidad_mes' => (int)$result['cantidad_mes']
        ];
...
// Total de ventas (suma de todas las ventas activas) - CORREGIDA
$query = "SELECT COALESCE(SUM(TOTAL_VENTA), 0) as total 
         FROM TBL_FACTURA 
         WHERE ESTADO_FACTURA = 'ACTIVA'";  // Quitamos la restricción del mes
$stmt = $this->conn->prepare($query);
$stmt->execute();
